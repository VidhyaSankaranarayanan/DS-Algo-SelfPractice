...
public class SmallestNegativeBalance {
	...
	/*
		 ...
		 * Constraint: 
		 * Test Data:
		 * Positive: {"Alex Blake 2, Blake Alex 2, Casey Alex 5, Blake Casey 7, Alex Blake 4, Alex Casey 4"}

		 * Output: "Casey"
		 * Edge: {"Alex Blake 4, Blake Alex 4"}
		 * Output: "Nobody has a negative balance"
		 * Negative: {"Alex Blake 4"}
		 * Output: Alex
		 * 
		 * Approach: 
		 * 
	...
	public void test2() {
		...
		Assert.assertTrue(Arrays.equals(new String[] {"Nobody ", "has ", "a ", "negative ", "balance"}, findSmallestNegativeBalance(str)));
	}
	...
	/*
	 * Pseudo code
	 * Create a tree map -> Key as string, Value as Integer
	 * Traverse through the string array
	 * 		Initialize the current string in temp
	 * 			Convert the temp to string array debt by splitting with space
	 * 			borrower = debt[0],lender = debt[1], amount = debt[2] 
	 * 			If map not contains borrower, Add borrower as a key and update the value - amount
	 * 			Else update the value
	 * 			If map not contains lender, Add lender as a key and update the value + amount
	 * 			Else update the value	
	 * Find min
	 * Create a list
	 * Traverse through the map and add the keys of min to the list
	...
	private String[] findSmallestNegativeBalance(String[] str) {
		...
		if(min >= 0) return new String[] {"Nobody ", "has ", "a ", "negative ", "balance"};
